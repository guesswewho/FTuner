...
AutoSchedule(SearchPolicy search_policy, TuningOptions tuning_options) {
  ...
  // <efficient>
  if (search_policy->search_task->hardware_api->num_level != 0) {
    std::tie(states, inst_disp_map) = search_policy->EfficientSearch(measurer);
  } else {
  ...
  if (IsDynTask(search_policy->search_task)) {
    return DynWklDispatcher(search_policy->search_task, std::move(states),
    ...
    if (state.defined()) {
      ...
      return Array<ObjectRef>{state, sch_and_tensors.first, sch_and_tensors.second};
    } else {
      ...
      return Array<ObjectRef>{search_policy->search_task->compute_dag->init_state,
