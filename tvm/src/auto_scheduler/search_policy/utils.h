...
// <efficient>
inline std::vector<int> GetPrimNumber(std::vector<int> wkl_insts) {
  std::vector<bool> is_prim_number(128);
  for (int i = 0; i < 128; i++) {
    is_prim_number[i] = true;
  }
  std::vector<int> prim_number;
  for (int i = 2; i < 128; i++) {
    if (is_prim_number[i]) {
      for (int j = 2; i * j < 128; j++) {
        is_prim_number[i * j] = false;
      }
      prim_number.push_back(i);
    }
  }
  std::vector<int> number_for_wkl;
  std::set<int> res;
  for (int i = 0; i < prim_number.size(); i++) {
    for (auto var : wkl_insts) {
      if (floor_by(var, prim_number[i]) * 1.0 / var < 1.05) {
        for (int j = 1; prim_number[i] * j <= 128; j++) {
          if (floor_by(var, prim_number[i] * j) * 1.0 / var < 1.05) {
            res.insert(prim_number[i] * j);
          }
        }
      }
    }
  }
  number_for_wkl.assign(res.begin(), res.end());
  return std::move(number_for_wkl);
}

// <efficient>
inline int GetParallelism(std::vector<int> large_tile, std::vector<int> small_tile) {
  int num_threads = 1;
  for (int i = 0; i < large_tile.size(); i++) {
    num_threads *= large_tile[i] / small_tile[i];
  }
  return num_threads;
}
...
// <bojian/DietCode>
inline bool IsDynTask(const SearchTask& task) { return (task)->shape_vars != nullptr; }

// <efficient>
inline bool IsEfficientTask(const SearchTask& task) { return (task)->hardware_api->num_level != 0; }
  ...
  if (op->IsInstance<te::ComputeOpNode>()) {
    ...
    std::tie(cum_space_len, cum_reduce_len) =
        GetCumulativeSpaceAndReductionLength(state->stages[stage_id]
                                             ...
                                             // <bojian/DietCode>
                                             ,
                                             task->shape_vars.value(), task->wkl_insts
...
// <efficient>
inline Iterator GetLastReduceIteratorForRegTile(const Stage& stage) {
  Iterator reduce_iter;
  for (const auto& iter : stage->iters) {
...
inline bool operator==(const SplitStepInfo& LHS, const SplitStepInfo& RHS) {
  return LHS.is_spatial == RHS.is_spatial && LHS.max_extent == RHS.max_extent;
}
...
struct FactorizationScheme {
  ...
  void RandomSample(const std::vector<SplitStepInfo>& split_steps_info,
                    const HardwareParams& hardware_param, const size_t max_innermost_factor,
...
class DietCodeSplitFactorizationMemo {
 ...
 public:
   ...
   */
  FactorizationScheme SampleFactorizationScheme(const std::vector<SplitStepInfo>& split_steps_info,
                                                std::mt19937* const rng);

  FactorizationScheme MutateFactorizationScheme(
...
std::vector<std::pair<State, int>> DenseAddMultilevelTile(const SearchTask& search_task,
                                                          const State& state, int stage_id);

std::vector<std::pair<State, int>> DenseMultilevelTile(const SearchTask& search_task,
                                                       const State& state, int stage_id);
