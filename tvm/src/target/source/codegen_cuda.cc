...
void CodeGenCUDA::PrintExtraAttrs(const PrimFunc& f) {
  ...
  PrimExpr blockIdx_ext = analyzer.Simplify(extractor.blockIdx_x_ext * extractor.blockIdx_y_ext *
  ...
  if (const IntImmNode* const threadIdx_ext_int = threadIdx_ext.as<IntImmNode>()) {
    ...
    if (const IntImmNode* const blockIdx_ext_int = blockIdx_ext.as<IntImmNode>()) {
      if (blockIdx_ext_int->value == 1) {
        return;
      }
      int64_t max_blocks_per_sm = dmlc::GetEnv("MAX_BLOCKS_PER_SM", 0);
      int64_t sm_num = dmlc::GetEnv("GPU_SM_NUM", 0);
      if (max_blocks_per_sm > 0 && sm_num > 0) {
        int64_t blocks_launch_bounds =
            std::min(max_blocks_per_sm, blockIdx_ext_int->value / sm_num);
        LOG(INFO) << blocks_launch_bounds;
        stream << " __launch_bounds__(" << threadIdx_ext_int->value << ", " << blocks_launch_bounds
               << ")";
      }else{
