...
from .fixture import PyTorchBERTFixture, PyTorchBERTLargeFixture
...
def test_infer_dynT():
    ...
    for i, T in enumerate(list(range(5, 128, 19)) + [128]):
        dietcode_auto_scheduler.infer(fmodel_fixture=PyTorchBERTFixture, model_args=(B, T),
                                      ...
                                      ansor_sched_log_fname=curr_working_dir + '/saved_schedules_G4/ansor_autosched_gpt2_16x{}.json'.format(T),
                                      append_log=False if i == 0 else True
                                      )

@tvm_dev_decor
def test_infer_large_dynT():
    B = 16

    curr_working_dir = os.path.dirname(os.path.abspath(__file__))

    for i, T in enumerate(list(range(5, 128, 19)) + [128]):
        dietcode_auto_scheduler.infer(fmodel_fixture=PyTorchBERTLargeFixture, model_args=(B, T),
                                      dietcode_sched_log_dir=curr_working_dir + '/../../ops/',
                                      ansor_sched_log_fname=curr_working_dir + '/saved_schedules_G4/ansor_autosched_gpt2_16x{}.json'.format(T),
                                      append_log=False if i == 0 else True
                                      )