...
// <efficient>
/********** RuleAlignHardwareTile**********/

SketchGenerationRule::ConditionKind RuleAlignHardwareTileWithFusion::MeetCondition(
    const SketchPolicyNode& policy, const State& state, int stage_id) const {
  if (NeedsMultilevelTiling(policy.search_task, state, stage_id) &&
    ...
    if (IsEfficientTask(policy.search_task)) {
      LOG(INFO) << "Align hardware tiling for dynamic workloads";
    }
...
std::vector<std::pair<State, int>> RuleAlignHardwareTileWithFusion::Apply(
    const SketchPolicyNode& policy, const State& state, int stage_id) const {
  int target_stage_id, first_target_stage_id;
  if (HasSingleElementwiseMatchedConsumer(policy.search_task, state, stage_id,
                                          &first_target_stage_id) &&
      HasSingleElementwiseMatchedConsumer(policy.search_task, state, first_target_stage_id,
                                          &target_stage_id)) {
    return DenseAddMultilevelTile(policy.search_task, state, stage_id);
  } else {
    return DenseMultilevelTile(policy.search_task, state, stage_id);
  }
}
...
std::vector<std::pair<State, int>> RuleAddCacheRead::Apply(const SketchPolicyNode& policy,
  ...
  for (int orig_target_stage_id : consumers) {
    ...
    // if (policy.search_task->hardware_api->num_level != 0) {
    //   int reg_stage_id =
    //       tmp_s.cache_read(added_stage_id, "local", {target_stage_id}, task->compute_dag);
    //   LOG(INFO) << tmp_s;
    //   target_stage_id_offset++;
    //   target_stage_id++;
    //   const auto& reg_read_pos = GetLastReduceIteratorForRegTile(tmp_s->stages[target_stage_id]);
    //   tmp_s.compute_at(reg_stage_id, target_stage_id, reg_read_pos);
    // }
  }
  LOG(INFO) << tmp_s;
...
SketchGenerationRule::ConditionKind RuleAddCacheWrite::MeetCondition(const SketchPolicyNode& policy,
  ...
  if (NeedsMultilevelTiling(policy.search_task, state, stage_id) &&
      !StrEndsWith(state->stages[stage_id]->op->name, ".local") && policy.search_task->hardware_api->num_level!=0) {
    return IsGPUTask(policy.search_task) ? ConditionKind::kApplyAndSkipRest : ConditionKind::kApply;
  }
  return ConditionKind::kSkip;
}
...
std::vector<std::pair<State, int>> RuleAddRfactor::Apply(const SketchPolicyNode& policy,
  ...
  // TODO(merrymercy): We can do more analysis here to generate less and more efficient
  // sketches. In some cases, we only need rfactor for more parallel In some cases, we only
  // need rfactor for vectorization. Now we will generate two versions and let the search
  // figure out the bette one.
...
SketchGenerationRule::ConditionKind RuleCrossThreadReduction::MeetCondition(
  ...
  if (op->IsInstance<te::ComputeOpNode>()) {
    ...
    std::tie(cum_space_len, cum_reduce_len) = GetCumulativeSpaceAndReductionLength(
        state->stages[stage_id]

        // <bojian/DietCode>
        ,
        policy.search_task->shape_vars.value(), policy.search_task->wkl_insts
...
// <efficient>
EfficientGenerationRule::ResultKind InitEfficientTileSize::Apply(
    SketchPolicyNode* policy, State* state, hardware::HwAlignedConfig config) const {
  StateNode* pstate = state->CopyOnWrite();
  int reduce_idx = 0;
  int space_idx = 0;
  for (int i = 0; i < config.space_tiles.size() - 1; i++) {
    for (int j = 0; j < config.space_tiles[i].size(); j++) {
      config.space_tiles[i][j] /= config.space_tiles[i + 1][j];
    }
  }
  for (int i = 0; i < config.reduce_tiles.size() - 1; i++) {
    for (int j = 0; j < config.reduce_tiles[i].size(); j++) {
      config.reduce_tiles[i][j] /= config.reduce_tiles[i + 1][j];
    }
  }
  for (size_t step_id = 0; step_id < (*state)->transform_steps.size(); ++step_id) {
    if (auto ps = (*state)->transform_steps[step_id].as<SplitStepNode>()) {
      if (ps->lengths.size() == 2) {
        Array<runtime::Optional<Integer>> split_factor;
        for (int i = 0; i < config.reduce_tiles.size(); i++) {
          split_factor.push_back(Integer(config.reduce_tiles[i][reduce_idx]));
        }
        reduce_idx++;
        pstate->transform_steps.Set(step_id, SplitStep(ps->stage_id, ps->iter_id, ps->extent,
                                                       split_factor, ps->inner_to_outer));
      } else if (ps->lengths.size() == 3) {
        Array<runtime::Optional<Integer>> split_factor;
        if (config.space_tiles.size() == 2) {
          split_factor.push_back(Integer(config.space_tiles[1][space_idx]));
          split_factor.push_back(Integer(config.space_tiles[0][space_idx]));
          split_factor.push_back(Integer(1));
        }
        space_idx++;
        pstate->transform_steps.Set(step_id, SplitStep(ps->stage_id, ps->iter_id, ps->extent,
                                                       split_factor, ps->inner_to_outer));
...
EfficientGenerationRule::ResultKind InitEfficientThreadBind::Apply(
    SketchPolicyNode* policy, State* state, hardware::HwAlignedConfig config) const {
  *state = policy->search_task->compute_dag.InferBound(*state);
  StateNode* pstate = state->CopyOnWrite();
  std::vector<Iterator> to_fuse;
  int stage_id = pstate->stages.size() - 1;
  int start_id = 0;
  auto pop = pstate->stages[stage_id]->op.as<te::ComputeOpNode>();
  for (int i = start_id; i < pop->axis.size(); i++) {
  ...
  to_fuse.clear();
  start_id++;
  for (int i = start_id; i < pop->axis.size() + start_id; i++) {
    const auto& it = (*state)->stages[stage_id]->iters[i];
    if (!StrEndsWith(it->name, ".1")) {
      break;
    }
    state->bind(stage_id, it, IteratorAnnotation::kVThread);
    start_id++;
    // to_fuse.push_back(it);
  }
  ...
  int thread_num = threadidx_it->range->extent.as<IntImmNode>()->value;
  to_fuse.clear();
  for (stage_id = 0; stage_id < pstate->stages.size(); stage_id++) {
    if (pstate->stages[stage_id]->compute_at == ComputeAtKind::kIter &&
        StrEndsWith(pstate->stages[stage_id]->op->name, ".shared")) {
      Iterator fused = state->fuse(stage_id, (*state)->stages[stage_id]->iters);
      const auto& iters0 = state->split(stage_id, fused, {Integer(thread_num)});
      state->unroll(stage_id, iters0[0]);
      state->bind(stage_id, iters0[1], IteratorAnnotation::kThreadX);
    }
  }
  return ResultKind::kValid;
}

EfficientGenerationRule::ResultKind InitEfficientUnroll::Apply(
    SketchPolicyNode* policy, State* state, hardware::HwAlignedConfig config) const {
  std::vector<Iterator> to_fuse;
  for (int stage_id = 0; stage_id < (*state)->stages.size(); stage_id++) {
    if (HasReduceIter((*state)->stages[stage_id])) {
      for (int iter_id = 0; iter_id < (*state)->stages[stage_id]->iters.size(); iter_id++) {
        if ((*state)->stages[stage_id]->iters[iter_id]->iter_kind == IteratorKind::kReduction &&
            StrEndsWith((*state)->stages[stage_id]->iters[iter_id]->name, ".1")) {
          to_fuse.push_back((*state)->stages[stage_id]->iters[iter_id]);
        }
      }
      const auto& final_reduce = state->fuse(stage_id, to_fuse);
      state->unroll(stage_id, final_reduce);
    }
  }
  return ResultKind::kValid;
}
...
static std::vector<SplitStepInfo> GetSplitStepsInfoFromWklInst(
  ...
  for (const size_t split_step_id : split_step_ids) {
    const SplitStep& split_step = Downcast<SplitStep>((*state)->transform_steps[split_step_id]);
    if (split_step->lengths.size() != 4) {
      CHECK(split_step->lengths.size() == 2);
      split_steps_info.push_back(
          SplitStepInfo{false, GetIntImm(analyzer.Simplify(replacer(split_step->extent.value())))});
    } else {
      split_steps_info.push_back(
          SplitStepInfo{true, GetIntImm(analyzer.Simplify(replacer(split_step->extent.value())))});
    }
...
static std::vector<SplitStepInfo> GetSplitStepsInfoFromWklInsts(
    const State* const state, const std::vector<size_t>& split_step_ids,
    const Array<DynShapeVar>& shape_vars, const Array<Array<IntImm>>& wkl_insts) {
  std::vector<SplitStepInfo> split_steps_info;
  ...
  for (const size_t split_step_id : split_step_ids) {
    ...
    if (split_step->lengths.size() != 4) {
      CHECK(split_step->lengths.size() == 2);
      split_steps_info.push_back(SplitStepInfo{
          false,
          EvaluateRangeForAllWklInsts(split_step->extent.value(), shape_vars, wkl_insts).second});
    } else {
      split_steps_info.push_back(SplitStepInfo{
          true,
          EvaluateRangeForAllWklInsts(split_step->extent.value(), shape_vars, wkl_insts).second});
...
PopulationGenerationRule::ResultKind InitFillTileSize::Apply(SketchPolicyNode* policy, State* state,
  ...
  if (IsDynTask(policy->search_task)) {
    ...
    std::vector<SplitStepInfo> split_steps_info = GetSplitStepsInfoFromWklInsts(
        state, split_step_ids, policy->search_task->shape_vars.value(),
        policy->search_task->wkl_insts);
    if (split_steps_info.empty()) {
      LOG(FATAL) << "split_steps_info is empty in state=" << state->ToStr()
                 << " with "
                    "transform_steps="
    ...
    try {
      scheme = policy->dietcode_split_memo.SampleFactorizationScheme(split_steps_info, rand_gen);
    ...
    // const FactorizationScheme& scheme = cached_schemes[(*rand_gen)() %
    // cached_schemes.size()];
    ...
    for (size_t step_id = 0, iter_id = 0; step_id < (*state)->transform_steps.size(); ++step_id) {
      if (auto ps = (*state)->transform_steps[step_id].as<SplitStepNode>()) {
        bool all_defined = true;
        for (const auto& len : ps->lengths) {
          if (!len) {
...
PopulationGenerationRule::ResultKind InitUnroll::Apply(SketchPolicyNode* policy, State* state,
  ...
  for (size_t stage_id = 0; stage_id < (*state)->stages.size(); ++stage_id) {
    ...
    if (stage->op->attrs.count(SearchPolicyKey::always_unroll_inner)) {
      ...
      // Unroll the space iterators and reduce iterators listed in the attrs in the
      // innermost tile
        ...
        for (size_t i = 0; i < (*state)->transform_steps.size(); ++i) {
          if (const SplitStepNode* const split_step =
            ...
            if (split_step->lengths.size() == 4) {
              ...
              unrolling_factor *=
                  (split_step->lengths[0].value()->value * split_step->lengths[3].value()->value);
            } else {
              CHECK(split_step->lengths.size() == 2);
  ...
  *state = policy->search_task->compute_dag.InferBound(*state);
  ...
  for (int stage_id = (*state)->stages.size() - 1; stage_id >= 0; --stage_id) {
    ...
    if (stage->compute_at == ComputeAtKind::kRoot) {
      ...
      // Otherwise, this is a tiled root stage, we assume it should be tiled with 3 space
      // level in the outer iterators. The remaining part deals with the thread binding for
      // multi-level tiled stages
      ...
      int max_total_space_extent =
          EvaluateRangeForAllWklInsts(total_space_extent, policy->search_task->shape_vars.value(),
                                      policy->search_task->wkl_insts)
              .second;
      if (enable_verbose_logging) {
        LOG(INFO) << "total_space_extent=" << total_space_extent
                  << " w/ max=" << max_total_space_extent;
      }
...
static PopulationGenerationRule::ResultKind AdjustUnrollingFactor(State* state) {
  ...
  for (size_t i = 0; i < (*state)->transform_steps.size(); ++i) {
    if (const SplitStepNode* const split_step = (*state)->transform_steps[i].as<SplitStepNode>()) {
      if (!split_step->extent.defined() ||
          (*state)->stages[split_step->stage_id]->op->name.find(".shared") != std::string::npos) {
        continue;
      }
      if (split_step->lengths.size() == 4) {
        CHECK(split_step->lengths[2].value()->value == 1);
...
PopulationGenerationRule::ResultKind MutateInnermostTileSize::Apply(SketchPolicyNode* policy,
                                                                    State* state,
                                                                    std::mt19937* rand_gen) const {
  ...
  Array<IntImm> selected_inst =
      policy->search_task->wkl_insts[RandomChoose(policy->curr_inst_opt_prob, rand_gen)];

  // LOG(INFO) << "Selected inst=" << ArrayToString(selected_inst);

  std::vector<SplitStepInfo> split_steps_info = GetSplitStepsInfoFromWklInst(
      state, split_step_ids, policy->search_task->shape_vars.value(), selected_inst);
  ...
  mutated_scheme = policy->dietcode_split_memo.MutateFactorizationScheme(split_steps_info, rand_gen,
                                                                         curr_split_factors);
  ...
  for (size_t i = 0; i < split_step_ids.size(); ++i) {
    ...
    pstate->transform_steps.Set(
        split_step_ids[i], SplitStep(split_step->stage_id, split_step->iter_id, split_step->extent,
                                     new_split_factor, split_step->inner_to_outer));
  }
  ...
  //   [1, 16, ]\n
  // ]") {
  //       LOG(WARNING) << "Current split_factors=" <<
  //       OptionalMatrixToString(state->GetSplitFactors()); LOG(WARNING) << "split factor
  //       captured during evolutionary search"; opt_tiling_detected = true;
...
PopulationGenerationRule::ResultKind MutateParallel::Apply(SketchPolicyNode* policy, State* state,
  ...
  for (size_t s = step_id + 1; s < (*state)->transform_steps.size(); ++s) {
    ...
    if (IsStageNumberChangingStep(step)) {
      ...
      // This is not an issue because this will never happend in normal cases where all
      // these steps are before parallel steps.
      return ResultKind::kInvalid;
    }
    tmp_s.CopyOnWrite()->transform_steps.push_back(step);
    try {
