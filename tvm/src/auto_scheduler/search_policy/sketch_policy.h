...
class SketchPolicyNode : public SearchPolicyNode {
 public:
  ...
  // <efficient>
  std::vector<EfficientGenerationRule*> efficient_init_rules;
 ...
 public:
  // <efficient>
  std::pair<std::vector<int>, std::vector<int>> GetNextConfig(
      std::vector<hardware::HwAlignedConfig>* pnext,
      std::vector<hardware::HwExprExtractor>& expr_extractor,
      hardware::HwAlignedConfig& base_config, int mem_level,
      std::vector<std::vector<int>::iterator>& space_ptr,
      std::vector<std::vector<int>::iterator>& reduce_ptr,
      std::vector<std::vector<int>>& space_tiles, std::vector<std::vector<int>>& reduce_tiles);
  // <efficient>
  std::pair<std::vector<std::vector<int>>, std::vector<std::vector<int>>> GetAlignedTile(
      std::vector<int> sbase_tile, std::vector<int> rbase_tile, int mem_level,
      std::vector<hardware::HwExprExtractor>& expr_extractor);
  // <efficient>
  double ComputeIntensiveThreshold(std::vector<int>& space_tiles, int mem_level);
  double GetComputeIntensiveRatio(std::vector<int>& space_tiles, std::vector<int>& reduce_tiles,
                                  int mem_level, int mem_use);
  // <efficient>
  void ConfigFilter(std::vector<hardware::HwAlignedConfig>* pnext,
                    hardware::HwAlignedConfig& base_config,
                    std::vector<hardware::HwExprExtractor>& expr_extractor,
                    std::vector<int>& space_tiles, std::vector<int>& reduce_tiles, int mem_level);
  //<efficient>
  std::pair<std::vector<State>, std::unordered_map<size_t, size_t>> EfficientSearch(
      ProgramMeasurer measurer) final;
  // <efficient>
  std::vector<hardware::HwAlignedConfig> EmitConfig(int space_dims, int reduce_dims);
  // <bojian/DietCode>
  // State
  // Array<ObjectRef>
  std::pair<std::vector<State>, std::unordered_map<size_t, size_t>> Search(
      int num_measure_trials, int early_stopping, int num_measures_per_round,
      ProgramMeasurer measurer) final;

  // <bojian/DietCode>
  // std::pair<Array<MeasureInput>, Array<MeasureResult>>
  std::pair<int, float> ContinueSearchOneRound(int num_measure, ProgramMeasurer measurer) final;
