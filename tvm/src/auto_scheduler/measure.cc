...
MeasureInput::MeasureInput(SearchTask task,
                           State state
...
Array<MeasureResult> ProgramMeasurerNode::Measure(const SearchTask& task,
    ...
    for (size_t j = 0; j < input_batch.size(); ++j) {
      ...
      if (result_batch[j]->error_no == 0) {
        ...
        if (IsDynTask(task)) {
          ...
          if (task->hardware_api->num_level != 0) {
            std::tie(cherry_picked_wkl_inst, flop_ct, adaption_penalty) =
                ...
                task->compute_dag.CherryPickAlignHardwareWorkloadInstance(input_batch[j]->state,
                                                                          task);
          } else {
            std::tie(cherry_picked_wkl_inst, flop_ct, adaption_penalty) =
                // GetSyntheticWorkloadFlopCtFromState(
                //   task, input_batch[j]->state);
                task->compute_dag.CherryPickWorkloadInstance(input_batch[j]->state, task);
          }
        ...
        flops =  // <bojian/DietCode>
            ...
            flop_ct / adaption_penalty / FloatArrayMean(result_batch[j]->costs);
        LOG(INFO) << input_batch[j]->state;
        LOG(INFO) << "Successfully completed the measurement on state w/ "
                     "split factors="
                  << OptionalMatrixToString(split_factors)
                  << ", "
                     "costs="
                  << result_batch[j]->costs
                  << "->"
                     "avg_cost="
                  ...
                  << " => "
                     "flops="
                  << flop_ct / FloatArrayMean(result_batch[j]->costs) << "->" << flops;
  ...
  if (IsDynTask(task)) {
    ...
    std::vector<float> adapted_candidate_flops(task->wkl_insts.size() * candidate_states.size());
    ...
    for (size_t state_id = 0; state_id < candidate_states.size(); ++state_id) {
      for (size_t inst_id = 0; inst_id < task->wkl_insts.size(); ++inst_id) {
        ...
        // <efficient>
        if (task->hardware_api->num_level != 0) {
          AlignHWAdaptStateToWorkload(
              task, candidate_states[state_id], task->wkl_insts[inst_id], candidate_flops[state_id],
              &occupancy_penalty, &padding_penalty,
              &adapted_candidate_flops[inst_id * candidate_states.size() + state_id]);
        } else {
    ...
    std::tie(inst_id_disp_map, selected_candidate_states, selected_candidate_flops,
        ...
        dispatcher.MapWklInstsToStates(raw_wkl_inst_id_disp_map, candidate_states, candidate_flops,
                                       task->wkl_insts, adapted_candidate_flops);
    ...
    for (const std::pair<size_t, size_t>& inst_state_pair : inst_id_disp_map) {
      inst_disp_map.Set(task->wkl_insts[inst_state_pair.first], Integer(inst_state_pair.second));
    }
    ...
    if (!prev_inst_flops.empty()) {
      for (size_t i = 0; i < task->wkl_insts.size(); ++i) {
        if (prev_inst_flops[i] > inst_predicted_flops[i]) {
          LOG(FATAL) << "Predicted FLOPS on inst=" << task->wkl_insts[i] << " dropped from "
