    ...
    if (op->IsInstance<te::PlaceholderOpNode>()) {
      ...
      for (const auto& pair : node->read_from[op]) {
        ...
        for (const auto& axis : cop->axis) {
          ...
          if ((pint == nullptr || pint->value > 1) && vars.count(axis->var.get()) == 0) {
            n_missing++;
            break;
          }
        }
...
TVM_STATIC_IR_FUNCTOR(ReprPrinter, vtable)
    .set_dispatch<te::PlaceholderOpNode>([](const ObjectRef& ref, ReprPrinter* p) {
      ...
      p->stream << "PlaceholderOpNode(" << node->name << ", shape=" << node->shape << ")";
    });
...
TVM_STATIC_IR_FUNCTOR(ReprPrinter, vtable)
    .set_dispatch<SplitStepNode>([](const ObjectRef& ref, ReprPrinter* p) {
      ...
      p->stream << "SplitStepNode("
                ...
                << ", "
                   "extent="
                << (node->extent == nullptr ? node->extent : node->extent.value())
                << ", "
                   "lengths="
                << OptionalArrayToString(node->lengths) << ")";
    });
...
Iterator FindIterInInitState(const State& init_state, const Iterator& iter_0) {
  for (size_t stage_id = 0; stage_id < init_state->stages.size(); ++stage_id) {
    ...
    for (const Iterator& iter : stage->iters) {
      if (std::string(iter_0->name).find(std::string(iter->name)) != std::string::npos) {
        return iter;
      }
    }
  }
...
// <bojian/DietCode>
// extern bool enable_verbose_logging;
std::pair<te::Schedule, Array<te::Tensor>> ComputeDAG::InstantiateAndApplySteps(
  ...
  return InstantiateAndApplySteps(state_mutable_copy, synthetic_expr_replacer, nullptr, nullptr);
}

std::tuple<Array<IntImm>, double, float> ComputeDAG::CherryPickAlignHardwareWorkloadInstance(
  ...
  float adapted_score, occupancy_penalty, padding_penalty, max_adapted_score = 0.;

  // std::vector<float> adapted_scores;

  for (const Array<IntImm>& wkl_inst : task->wkl_insts) {
    AlignHWAdaptStateToWorkload(task, state_mutable_copy, wkl_inst, base_score, &occupancy_penalty,
    ...
    if (adapted_score == max_adapted_score) {
      ...
      for (size_t i = 0; i < wkl_inst.size(); ++i) {
        if (wkl_inst[i]->value > cherry_picked_wkl_inst[i]->value) {
          if (enable_verbose_logging) {
            LOG(WARNING) << cherry_picked_wkl_inst << " <= " << wkl_inst;
          }
          cherry_picked_wkl_inst = wkl_inst;
          break;
        }
  ...
  double inst_flop =
      EstimateFlopForInst(task->compute_dag, task->shape_vars.value(), cherry_picked_wkl_inst);
  AlignHWAdaptStateToWorkload(task, state, cherry_picked_wkl_inst, base_score, &occupancy_penalty,
                              &padding_penalty, &adapted_score);
  // LOG(INFO) << cherry_picked_wkl_inst;
  // LOG(INFO) << inst_flop;
  return std::make_tuple(cherry_picked_wkl_inst, inst_flop, adapted_score);
}

std::tuple<Array<IntImm>, double, float> ComputeDAG::CherryPickWorkloadInstance(
    const State& state, const SearchTask& task) const {
  State state_mutable_copy = state;
  ...
  for (const Array<IntImm>& wkl_inst : task->wkl_insts) {
    AdaptStateToWorkload(task, state_mutable_copy, wkl_inst, base_score, &occupancy_penalty,
                         &padding_penalty, &adapted_score);
  ...
  double inst_flop =
      EstimateFlopForInst(task->compute_dag, task->shape_vars.value(), cherry_picked_wkl_inst);
  AdaptStateToWorkload(task, state, cherry_picked_wkl_inst, base_score, &occupancy_penalty,
                       &padding_penalty, &adapted_score);
...
std::pair<te::Schedule, Array<te::Tensor>> ComputeDAG::CherryPickWorkloadInstanceAndApplySteps(
    const State& state, const SearchTask& task) const {
  Array<IntImm> shape_values;
  if (task->hardware_api->num_level != 0) {
    shape_values = std::get<0>(CherryPickAlignHardwareWorkloadInstance(state, task));
  } else {
    shape_values = std::get<0>(CherryPickWorkloadInstance(state, task));
  }
  if (enable_verbose_logging) {
    LOG(INFO) << "Cherry picked workload inst=" << shape_values;
  }

  return InstantiateAndApplySteps(state, task->shape_vars.value(), ToPrimExprArray(shape_values));
}
...
//                                   nullptr, nullptr);
// }
void InstantiateRecursive(arith::Analyzer& analyzer, SyntheticExprReplacer& replacer,
                          Array<te::Tensor>& synthetic_tensors,
                          const te::ComputeOpNode* compute_op_node, const te::Tensor& t) {
  for (const te::Tensor& input_tensor : compute_op_node->InputTensors()) {
    if (const te::ComputeOpNode* input_op = input_tensor->op.as<te::ComputeOpNode>()) {
      InstantiateRecursive(analyzer, replacer, synthetic_tensors, input_op, input_tensor);
    }
  }
  ...
  for (const IterVar& iv : compute_op_node->axis) {
    synthetic_axis.push_back(
        tir::IterVar(Range::FromMinExtent(analyzer.Simplify(replacer(iv->dom->min)),
                     ...
                     iv->var, iv->iter_type, iv->thread_tag));
  }
  ...
  te::Operation synthetic_op =
      te::ComputeOp(compute_op_node->name, compute_op_node->tag, compute_op_node->attrs,
                    synthetic_axis, synthetic_body);
  CHECK(synthetic_op.defined());
  // LOG(INFO) << "Original op=" << t->op << " vs.";
  // LOG(INFO) << "Synthetic op=" << synthetic_op;
  synthetic_tensors.push_back(te::Tensor(synthetic_shape, t->dtype, synthetic_op, t->value_index));
  replacer.producer_subst_map.Set(t, synthetic_tensors.back());
}

std::pair<te::Schedule, Array<te::Tensor>> ComputeDAG::InstantiateAndApplySteps(
    const State& state, SyntheticExprReplacer& replacer, Array<te::Stage>* stages,
  ...
  for (const te::Tensor& t : operator->()->tensors) {
    ...
    const te::PlaceholderOpNode* placeholder_op_node = t->op.as<te::PlaceholderOpNode>();
    if (compute_op_node != nullptr) {
      InstantiateRecursive(analyzer, replacer, synthetic_tensors, compute_op_node, t);
    } else if (placeholder_op_node != nullptr) {
      synthetic_op =
          te::PlaceholderOp(placeholder_op_node->name, synthetic_shape, placeholder_op_node->dtype);
      CHECK(synthetic_op.defined());
      // LOG(INFO) << "Original op=" << t->op << " vs.";
      // LOG(INFO) << "Synthetic op=" << synthetic_op;
      synthetic_tensors.push_back(
          te::Tensor(synthetic_shape, t->dtype, synthetic_op, t->value_index));
      replacer.producer_subst_map.Set(t, synthetic_tensors.back());
    }
  }
  ...
  for (const Step& step : state->transform_steps) {
    ...
    StepApplyToSchedule(step, stages, stage_to_axes, &synthetic_sch, state->transform_steps);
  }
...
TVM_REGISTER_GLOBAL("auto_scheduler.PrintStateSplitFactors")
    .set_body_typed([](const ComputeDAG& dag, const State& state) -> void {
...
TVM_REGISTER_GLOBAL("auto_scheduler.CherryPickWorkloadInstance")
    .set_body_typed([](const ComputeDAG& dag, const State& state,
                       const SearchTask& task) -> Array<ObjectRef> {
...
TVM_REGISTER_GLOBAL("auto_scheduler.GetSchedArgsPairOnWklInst")
    .set_body_typed([](const ComputeDAG& dag, const State& state,
                       const Array<DynShapeVar>& shape_vars, const Array<IntImm>& wkl_inst) {
