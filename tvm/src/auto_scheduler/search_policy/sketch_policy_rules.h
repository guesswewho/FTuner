...
//<efficient>
DEFINE_SKETCH_GENERATION_RULE(RuleAlignHardwareTileWithFusion)
...
class EfficientGenerationRule {
 public:
  ...
  virtual ResultKind Apply(SketchPolicyNode* policy, State* state,
                           hardware::HwAlignedConfig config) const = 0;

  /*! \brief The deconstructor */
  virtual ~EfficientGenerationRule() = default;
};

// A helper to define population initialization rules
#define DEFINE_INIT_EFFICIENT_RULE(rule_name)                                                    \
  class rule_name : public EfficientGenerationRule {                                             \
   public:                                                                                        \
    ResultKind Apply(SketchPolicyNode* policy, State* state, hardware::HwAlignedConfig config) const final; \
  };

DEFINE_INIT_EFFICIENT_RULE(InitEfficientTileSize);

DEFINE_INIT_EFFICIENT_RULE(InitEfficientThreadBind);

DEFINE_INIT_EFFICIENT_RULE(InitEfficientUnroll);
