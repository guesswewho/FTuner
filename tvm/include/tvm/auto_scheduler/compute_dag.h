...
class ComputeDAG : public ObjectRef {
 ...
 private:
  // <bojian/DietCode>
  std::pair<te::Schedule, Array<te::Tensor>> InstantiateAndApplySteps(
      const State& state, SyntheticExprReplacer& replacer, Array<te::Stage>* stages,
      StageToAxesMap* stage_to_axes) const;

 public:
  // <efficient>
  std::tuple<Array<IntImm>, double, float> CherryPickAlignHardwareWorkloadInstance(
      const State& state, const SearchTask& task) const;
  // <bojian/DietCode>
  std::tuple<Array<IntImm>, double, float> CherryPickWorkloadInstance(const State& state,
                                                                      const SearchTask& task) const;
  std::pair<te::Schedule, Array<te::Tensor>> CherryPickWorkloadInstanceAndApplySteps(
      const State& state, const SearchTask& task) const;
