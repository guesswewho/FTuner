...
std::vector<std::pair<State, int>> DenseMultilevelTile(const SearchTask& search_task, const State& state, int stage_id) {
  int target_stage_id;
  ICHECK(
      HasSingleElementwiseMatchedConsumer(search_task, state, stage_id, &target_stage_id));
  std::vector<int> spatial_split_step_ids;
  State base_state = DoAlignHardwareTile(state, stage_id, search_task->hardware_api,
                                         &spatial_split_step_ids);
  std::vector<std::pair<State, int>> ret;
  int follow_tiling_levels = search_task->hardware_api->num_level < 3
                                 ? 3
                                 : search_task->hardware_api->num_level;
  State tmp_s = base_state;
  std::vector<std::vector<Iterator>> space_levels;
  space_levels.resize(follow_tiling_levels + 1);
  auto sr_levels = [&](int size, const Iterator& iter, std::vector<std::vector<Iterator>>& levels) {
    ICHECK_GE(size, 1);
    Array<Iterator> split_res =
        tmp_s.split(target_stage_id, iter, Array<Optional<Integer>>(size, NullOpt));
    for (int i = 0; i <= size; i++) {
      levels[i].push_back(split_res[i]);
    }
  };
  int split_nums = 0;
  for (auto& iter : tmp_s->stages[target_stage_id]->iters) {
    sr_levels(follow_tiling_levels, iter, space_levels);
    split_nums++;
  }
  Array<Iterator> order;
  int space_ct = 0;
  for (int i = 0; i <= follow_tiling_levels; i++) {
  ...
  tmp_s.reorder(target_stage_id, order);
  const Iterator& target_iter =
      tmp_s->stages[target_stage_id]->iters[follow_tiling_levels * split_nums - 1];
...
std::vector<std::pair<State, int>> DenseAddMultilevelTile(const SearchTask& search_task, const State& state, int stage_id) {
  int first_target_stage_id, target_stage_id;
  ICHECK(
      HasSingleElementwiseMatchedConsumer(search_task, state, stage_id, &first_target_stage_id));
  ICHECK(
        HasSingleElementwiseMatchedConsumer(search_task, state, first_target_stage_id, &target_stage_id));
  std::vector<int> spatial_split_step_ids;
  State base_state = DoAlignHardwareTile(state, stage_id, search_task->hardware_api,
                                         &spatial_split_step_ids);
  std::vector<std::pair<State, int>> ret;
  ...
  const Iterator& target_iter =
      tmp_s->stages[target_stage_id]->iters[follow_tiling_levels * split_nums - 1];
  tmp_s.compute_at(stage_id, target_stage_id, target_iter);
  tmp_s.compute_at(first_target_stage_id, target_stage_id, target_iter);
  LOG(INFO) << tmp_s;
  ret.emplace_back(std::move(tmp_s), stage_id - 1);
  return ret;
}
...
//<efficient>
State DoAlignHardwareTile(const State& state, int stage_id, hardware::HardwareAPI hardware_api,
                          std::vector<int>* spatial_split_step_ids) {
  ...
  space_levels.resize((hardware_api->num_level < 3 ? 3 : hardware_api->num_level)+1);
  reduce_levels.resize(hardware_api->num_level+1);
  State tmp_s = state;
  ...
  auto sr_levels = [&](int size, const Iterator& iter, std::vector<std::vector<Iterator>>& levels) {
    ...
    if (size == 0) {
      ...
      Array<Iterator> split_res =
          tmp_s.split(stage_id, iter, Array<Optional<Integer>>(size, NullOpt));
      for (int i = 0; i <= size; i++) {
        levels[i].push_back(split_res[i]);
      }
  ...
  for (const auto& iter : state->stages[stage_id]->iters) {
    if (!no_split_at_inner_name_set.count(iter->name)) {
      if (iter->iter_kind == IteratorKind::kSpatial) {
        sr_levels(0, iter, space_levels);
      } else if (iter->iter_kind == IteratorKind::kReduction) {
        sr_levels(hardware_api->num_level, iter, reduce_levels);
      } else {
  ...
  for (int i = 0; i <= hardware_api->num_level; i++) {
...
static void shrink_below_max_extent(int& curr_split_factor, const size_t max_extent) {
  ...
  while (should_shrink_f) {
    ...
    if (floor_div(curr_split_factor, 2) > static_cast<int>(max_extent * 1.1)) {
      if (enable_verbose_logging) {
        LOG(INFO) << "Shrinking orig_split_factor=" << curr_split_factor
                  << " to "
                     "new_split_factor="
                  << curr_split_factor / 2;
      }
...
std::vector<int> mutate_split_factors(const std::vector<int>& input_split_factors,
                                      const int max_innermost_extent, std::mt19937* const rng,
...
void FactorizationScheme::RandomSample(const std::vector<SplitStepInfo>& split_steps_info,
                                       ...
                                       const bool do_mutation, const bool sample_perfect_tiles) {
  // ===========================================================================
  ...
  if (!do_mutation) {
    ...
    std::uniform_int_distribution<> num_threads_per_block_dist(0, C_NUM_THREADS_PER_BLOCK_SIZE - 1);
    ...
    for (Array<Integer> factor_scheme : num_threads_factor_schemes) {
      ...
      if (enable_verbose_logging) {
        LOG(INFO) << "Factorization Scheme for Threads Per Block: " << factor_scheme;
      }

      // check whether the factorization scheme is valid or not
      bool all_below_max_extents = true;
      for (size_t iter_id = 0, spatial_iter_id = 0; iter_id < split_steps_info.size(); ++iter_id) {
        if (split_steps_info[iter_id].is_spatial) {
          if (factor_scheme[spatial_iter_id]->value >
              static_cast<int>(split_steps_info[iter_id].max_extent * 1.1)) {
    ...
    for (size_t iter_id = 0, spatial_iter_id = 0; iter_id < split_steps_info.size(); ++iter_id) {
      if (split_steps_info[iter_id].is_spatial) {
        shrink_below_max_extent(split_factors[iter_id][1], split_steps_info[iter_id].max_extent);
        ++spatial_iter_id;
      }
    }
  ...
  auto sample_factors = [&](std::function<bool(const size_t)> fcontinue_predicate,
    ...
    for (size_t iter_id = 0; iter_id < split_steps_info.size(); ++iter_id) {
      ...
      const size_t max_extent = std::min(fmax_extent(iter_id), fextent_to_factor(iter_id));
      ...
      if (do_mutation) {
        ...
        if (split_steps_info[iter_id].is_spatial) {
          ...
          int curr_outer_factor = floor_div(fextent_to_factor(iter_id), curr_inner_factor);
          ...
          //                 << ArrayToString(std::vector<int>{curr_outer_factor,
          //                 curr_nthreads_factor,
          //                                                   curr_inner_factor}) << ", "
          ...
          std::vector<int> mutated_split_factors = mutate_split_factors(
              {curr_outer_factor, curr_inner_factor}, fmax_extent(iter_id), rng, memo);
      ...
      if (sample_perfect_tiles) {
        ...
        if (!filtered_extent_factors.empty() &&
          ...
          factor_to_assign = RandomChooseAmong(filtered_extent_factors, rng);
        }  // else {
  ...
  sample_factors(
      [&](const size_t iter_id) -> bool {
        return (!split_steps_info[iter_id].is_spatial) || (iter_id != last_spatial_iter_id);
      },
      ...
      [&](const size_t iter_id) -> int& { return split_factors[iter_id][0]; },
      [&](const size_t iter_id) -> size_t {
        return floor_div(split_steps_info[iter_id].max_extent, split_factors[iter_id][1]);
      });
  ...
  sample_factors(
      ...
      [&](const size_t iter_id) -> int& { return split_factors[iter_id][3]; },
      [&](const size_t iter_id) -> size_t {
        return floor_div(split_steps_info[iter_id].max_extent,
                         split_factors[iter_id][0] * split_factors[iter_id][1]);
      });
  if (enable_verbose_logging) {
    LOG(INFO) << "Finished sampling the innermost loop extent, current scheme=" << toString();
  }
  ...
  //       )
  //     ) {
  //   // LOG(FATAL) << "shmem_usage=" << shmem_usage << " is already greater than the allowable
  //   size "
  ...
  // ===========================================================================
  sample_factors(
      [&](const size_t iter_id) -> bool { return split_steps_info[iter_id].is_spatial; },
      [&](const size_t iter_id) -> size_t {
        // max_innermost_extent =
        //     std::min(max_innermost_extent,
      ...
      [&](const size_t iter_id) -> int& { return split_factors[iter_id][1]; },
      [&](const size_t iter_id) -> size_t { return split_steps_info[iter_id].max_extent; });
  if (enable_verbose_logging) {
...
FactorizationScheme DietCodeSplitFactorizationMemo::SampleFactorizationScheme(
  ...
  scheme.RandomSample(split_steps_info, hardware_params_, max_innermost_factor_, rng,
                      false,  // do_mutation
                      true);
...
FactorizationScheme DietCodeSplitFactorizationMemo::MutateFactorizationScheme(
    const std::vector<SplitStepInfo>& split_steps_info, std::mt19937* const rng,
